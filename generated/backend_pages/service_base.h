// АВТОСГЕНЕРИРОВАННЫЙ ФАЙЛ.
// Источник: eez_project/src/ui/screens.h + eez_project/src/ui/screens.c
// НЕ РЕДАКТИРОВАТЬ ВРУЧНУЮ. Запуск: python tools/ui_meta_gen/generate_ui_meta.py

#pragma once

#include <stdint.h>

#include "Screen/Screen.h"
#include "pages/IHostPage.h"

#include "../shared/element_ids.generated.h"
#include "../shared/page_ids.generated.h"

namespace screenui {

template <typename TPage>
class ServiceBase : public screenlib::IHostPage {
public:
    static constexpr uint32_t kPageId = scr_SERVICE;
    // Открывает страницу через системный фасад экрана.
    static bool show() {
        return machine32::screen::Screen::getInstance().showPage<TPage>();
    }
    uint32_t pageId() const final { return kPageId; }

protected:
    // === Кнопки ===
    virtual void onClickServiceBack() {}
    virtual void onClickNext2() {}
    virtual void onClickServiceTable() {}
    virtual void onClickServicePaper() {}
    virtual void onClickServiceGuillotine() {}
    virtual void onClickServiceSlice() {}
    virtual void onClickServiceCalibration() {}
    virtual void onClickServiceProba() {}

private:
    void onButton(uint32_t elementId) final {
        switch (elementId) {
            case btn_SERVICE_BACK: onClickServiceBack(); break;
            case btn_NEXT_2: onClickNext2(); break;
            case btn_SERVICE_TABLE: onClickServiceTable(); break;
            case btn_SERVICE_PAPER: onClickServicePaper(); break;
            case btn_SERVICE_GUILLOTINE: onClickServiceGuillotine(); break;
            case btn_SERVICE_SLICE: onClickServiceSlice(); break;
            case btn_SERVICE_CALIBRATION: onClickServiceCalibration(); break;
            case btn_SERVICE_PROBA: onClickServiceProba(); break;
            default: break;
        }
    }

};

}  // namespace screenui
